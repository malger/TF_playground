...
class Model:
    def __init__(self):
        ...
        #input -> hidden
        self.t1 = tf.Variable(tf.random.truncated_normal(shape=[nodes[0],nodes[1]],stddev=0.1))
        #hidden -> target
        self.t2 = tf.Variable(tf.random.truncated_normal(shape=[nodes[1],nodes[2]],stddev=0.1))
    ...
    def predict(self,x):
        ...
        hiddenLayer = tf.math.add(tf.linalg.matmul(inputlayer,self.t1),self.b1)
        #relu function activation
        hiddenLayer.activation = tf.nn.relu(hiddenLayer)
        #outputlayer
        output = tf.math.add(tf.linalg.matmul(hiddenLayer.activation,self.t2),self.b2)
    ...
    def compute_metrices(self,x,y):
        ...
        y_pred_class = tf.reshape(tf.cast(tf.math.greater(y_pred,0.5),tf.float32),shape=y_train.shape)
